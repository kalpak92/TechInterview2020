...
public class MaximumPointsFromCards {
    public static int maxScore(int[] cardPoints, int k) {
        ...
        // Start adding the last k elements
        for(int i = cardPoints.length - k; i < cardPoints.length; i++)
            sum += cardPoints[i];

        // Let this be the best we get.
        result = sum;

        // Check from the beginning.
        // Add an element and remove the corresponding (n - k + i)th element from the sum.
        // this happens for i = [0..k]
        for(int i = 0; i < k; i++) {
